...
void Meshes::CreateSphereMesh(GLmesh& mesh) {
	...
	float u, v;
	vector<GLfloat> combined_values;

	// combine interleaved vertices, normals and texture coordinates
	...
	glBindBuffer(GL_ARRAY_BUFFER, mesh.vbo[0]); // Activates buffer
	glBufferData(GL_ARRAY_BUFFER, sizeof(GLfloat) * combined_values.size(), combined_values.data(), GL_STATIC_DRAW); // sends vertex or coordinate data to GPU
...
void Meshes::CreateSphereMesh2(void) {
	...
	glGenVertexArrays(numVAOs, &sphere.vao);
